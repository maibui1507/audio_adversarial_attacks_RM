...
class IFGSM(Attack):
    ...
    def __init__(self, model, device, eps=8/255, alpha=2/255, steps=5):
        super().__init__("IFGSM", model, device)
        self.eps = eps
        self.steps = steps
        self.alpha = alpha
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if adv_labels == labels:
                # print("IFGSM successfully attacked")
                break
            else:
                self.alpha = self.alpha * 1.2
        if adv_labels != labels:
            # print("IFGSM failed to attack")
            return None
