...
class PGDL2(Attack):
    ...
    """
    def __init__(self, model, device, eps=1.0, alpha=0.2, steps=40, random_start=True, eps_for_division=1e-10):
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if adv_labels == labels:
                # print("PGD L2 successfully attacked")
                break
            else:
                self.alpha = self.alpha * 1.2

        if adv_labels != labels:
            # print("PGD L2 failed to attack")
            return None
