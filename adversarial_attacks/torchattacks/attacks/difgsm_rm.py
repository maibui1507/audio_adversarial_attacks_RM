...
class DIFGSM_RM(Attack):
    ...
    def __init__(self, model, device, eps=8/255, alpha=2/255, steps=20, decay=0.0, probability=0.5,
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if adv_labels == labels:
                # print("DIFGSM RM successfully attacked")
                break
            else:
                self.alpha = self.alpha * 1.2
        if adv_labels != labels:
            # print("DIFGSM RM failed to attack")
            # return adv_images
            return None
