...
class AdversarialGDTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attacks = None
        self.output_path = "/datab/Dataset/maibui/random_masking/"
    ...
    def generate_adversarial_samples(self, attack_model, adversarial_attacks, dataset):
        attack_model.eval()
        adversarial_samples = []
        for batch_x, _, batch_y in dataset:
            batch_x = batch_x.to(self.device)
            batch_y = batch_y.unsqueeze(1).type(torch.float32).to(self.device)

            self.init_adv_attacks(attack_model, adversarial_attacks)
            adversarial_sample = self.apply_adv_attack(batch_x, batch_y)

            #save adversarial samples as audio file
            adversarial_sample.export(os.path.join(self.output_path, dataset.filename + "_" + adversarial_attacks[0] + ".flac"), format="flac")
